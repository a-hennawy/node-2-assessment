...
/**the beforeEach function was replaced by beforeAll, since the database will remain empty
 */
/** beforeAll tests, insert u1, u2, and u3  [u3 is admin] */
...
//     /**added an admin key/value pair in the req. */
...
//   test("Should return error 401, if credentials are not matching", async () => {
//     const response = await request(app).post("/auth/login").send({
//       username: "u2",
//       password: "pwd1",
//     });
...
//   });
//   test("Should return error 404, if user not found", async () => {
//     const response = await request(app).post("/auth/login").send({
